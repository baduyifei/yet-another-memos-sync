...
1. 暂时把“同步天数限制”设为 `0`。
2. 执行一次“强制同步所有备忘录”。
3. 检查历史 Memo 的块 ID 已由 `^数字时间戳` 变为 `^UID`。
4. 再把同步天数恢复为平时使用的范围。
...
| 同步天数限制 | `30` | 所有同步模式都遵守；`0` 表示处理全部历史 |
...
1. 将“同步天数限制”设为需要补录的范围；设为 `0` 才会处理全部历史 Memos。
2. 执行 **强制同步所有备忘录**。
...
同步天数是从当前时间向前计算的自然时间范围，不是“最近有内容的 N 天”。智能、增量和强制同步都会遵守此限制；设置为 `0` 才表示不限制历史范围。
...
改变天数后，需要重新执行强制同步才能对新的时间范围执行创建、更新、移动和删除对齐。
...
| 强制同步所有备忘录 | 在“同步天数限制”内按 UID 创建、更新、跨日移动，并删除远端已不存在的本地 Memo 块；`0` 表示全部历史 |
| 打开设置 | 打开插件设置页 |

强制同步只修改 Memos 管理的标题区块；历史日记的其他内容会保留。同 UID 的正文和时间会更新到远端当前状态，日期变化时从旧 Daily Note 移动到新 Daily Note，远端删除时本地块也会删除，同步天数范围内最终与 Memos 当前状态一致。下载过的附件文件继续保留。

## 常见问题

### Memos 已删除，但 Obsidian 仍然存在

升级到 1.6.9 或更高版本后执行“强制同步所有备忘录”。智能/增量同步不会执行删除。被删除的 Memo 必须位于当前“同步天数限制”内；更早的数据需要临时扩大天数或设置为 `0`。如果多个账户使用相同标题，请先为每个账户配置不同标题，否则安全机制会禁用删除清理。
