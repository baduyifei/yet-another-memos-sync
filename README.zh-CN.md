...
- 每条 Memo 使用 Memos API 的永久 UID，例如 `^j3czVVLfdaXpHNwTitnB7B`。
- UID 不随正文、创建时间或日期修改而改变。
- 同步前检查全部 Daily Notes，而不是只检查当前标题或当前日期文件。
- UID 已存在：保留第一次出现的本地备份，忽略同 UID 云端记录，不覆盖也不重复追加。
- UID 不存在：在配置的 Memos 标题下创建一次备份。
...
| 强制同步所有备忘录 | 忽略上次同步时间，补齐缺失 Memo，迁移旧时间戳 ID，并按 UID 合并历史重复项 |
| 打开设置 | 打开插件设置页 |
