...
### 4. Memo ID 全文件唯一与重复自动修复

- 每条 Memo 末尾的数字块 ID（例如 `^1783037940`）是唯一标识。
- 同步前检查整篇 Daily Note，而不是只检查某一个标题区块。
- ID 已存在：保留第一次出现的本地备份，忽略同 ID 云端记录，不覆盖也不重复追加。
- ID 不存在：在配置的 Memos 标题下创建一次备份。
- `Today's Memos`、`Today‘s Memos` 和 `Today’s Memos` 会被识别为同一个标题。
- 如果历史文件已有多个等价标题或重复 ID，下一次强制同步会合并为一个标题、一个 Memo。
...
| 强制同步所有备忘录 | 忽略上次同步时间，补齐缺失 Memo，并按块 ID 合并历史重复项 |
| 打开设置 | 打开插件设置页 |

强制同步只影响 Memos 管理的标题区块；历史日记的其他内容会保留。已经存在的 Memo ID 采用只插入一次的备份规则，不会被覆盖或删除。
